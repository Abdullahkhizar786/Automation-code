...
describe("Demo site test", () => {
  it("to verify that new user can enter new data in table", () => {
    ...
    cy.wait(5000);
    cy.toFillFoam("Abdullah", "khizar", "abdullahkhizar05@gmail.com", "21", "221", "QA Automation");
    // cy.url().should("include", "demo");
    // cy.scrollTo("center");
    // cy.wait(3000);
    // cy.get(DemoNav.toclickonElement).eq(0).should("have.class", "card mt-4 top-card").click();
  ...
  it("Verify hte progress bar", () => {
    cy.visit("https://demoqa.com/");
    cy.get(DemoNav.tolickonWidgets).eq(3).should("be.visible").click();
    cy.get(DemoNav.toclickonslider).click();
    cy.get(DemoNav.toclickonstart).should("have.class", "mt-3 btn btn-primary").click();
  });

  it("Verifiy the tooltip", () => {
    cy.get(DemoNav.tolickonWidgets).eq(3).should("be.visible").click();
    cy.get(DemoNav.toclickonToolTips).should("be.visible").click();
    // cy.scrollTo("center");
    cy.wait(3000);
    cy.get(DemoNav.toselecthower).should("be.visible").trigger("mouseover");
  });

  it("verify user can drag and drop", () => {
    cy.get(DemoNav.toclickonInteractions).eq(4).should("be.visible").click();
    cy.get(DemoNav.toclickonDroppable).should("be.visible").click();
    cy.wait(3000);
    cy.scrollTo("center");
